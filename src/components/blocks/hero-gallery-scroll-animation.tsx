...
const BentoCell = React.forwardRef<HTMLDivElement, HTMLMotionProps<"div">>(
  ({ className, style, ...props }, ref) => {
    ...
    const translate = useTransform(scrollYProgress, [0.1, 0.75], ["-35%", "0%"])
...
const ContainerScale = React.forwardRef<HTMLDivElement, HTMLMotionProps<"div">>(
  ({ className, style, ...props }, ref) => {
    const { scrollYProgress } = useContainerScrollContext()
    const opacity = useTransform(scrollYProgress, [0.1, 0.4, 0.6, 0.75], [0, 1, 1, 0])
    const scale = useTransform(scrollYProgress, [0.1, 0.4, 0.6, 0.75], [0.8, 1, 1, 0.8])

    const position = useTransform(scrollYProgress, (pos) =>
      pos >= 0.1 && pos <= 0.75 ? "fixed" : "absolute"
    )
    return (
      <motion.div
        ref={ref}
